...
export const useHelpPosts = () => {
  ...
  const addHelpPost = (newPost: HelpPost) => {
    ...
    if (helpPosts.some(post => post.id === newPost.id)) {
      ...
      addLogEntry(t("common.attemptToAddExistingHelpPost"), { postId: newPost.id }, user?.username);
      return false;
    }
    setHelpPosts((prev) => [...prev, newPost]);
    toast.success(t("common.helpPostAddedSuccess", { postId: newPost.id }));
    addLogEntry(t("common.helpPostAdded"), { postId: newPost.id, title: newPost.title }, user?.username);
    return true;
  };
  ...
  const updateHelpPost = (updatedPost: HelpPost) => {
    ...
    addLogEntry(t("common.helpPostUpdated"), { postId: updatedPost.id, title: updatedPost.title }, user?.username);
    return true;
  };
  ...
  const deleteHelpPost = (id: string) => {
    ...
    addLogEntry(t("common.helpPostDeleted"), { postId: id }, user?.username);
    return true;
  };
